...
const AMserver = {
    ...
    AMmapChars: (node, variant, inputsym) => {
        ...
        if (tag == "MI" || tag == "MO" || tag == "MN" || tag == "MTEXT") {
            ...
            node.replaceChild(AMserver.createTextNode(newst), node.firstChild);
        }
        else {
            for (let i = 0; i < node.childNodes.length; i++) {
                AMserver.AMmapChars(node.childNodes[i], variant, inputsym);
            }
        }
    },
    AMparseIexpr: (str) => {
        ...
        if (symbol.ttype == INFIX && symbol.input != "/") {
            ...
            if (result[0] == null) // show box in place of missing argument
                result[0] = AMserver.createMmlNode("mo", AMserver.createTextNode("\u25A1"));
            else
                AMserver.AMremoveBrackets(result[0]);
            str = result[1];
            ...
            if (symbol.input == "_") {
                sym2 = AMserver.AMgetSymbol(str);
                if (sym2.input == "^") {
                    str = AMserver.AMremoveCharsAndBlanks(str, sym2.input.length);
                    let res2 = AMserver.AMparseSexpr(str);
                    AMserver.AMremoveBrackets(res2[0]);
                    str = res2[1];
                    node = AMserver.createMmlNode((underover ? "munderover" : "msubsup"), node);
                    node.appendChild(result[0]);
                    node.appendChild(res2[0]);
                    node = AMserver.createMmlNode("mrow", node); // so sum does not stretch
                }
                else {
                    node = AMserver.createMmlNode((underover ? "munder" : "msub"), node);
                    node.appendChild(result[0]);
                }
            }
            else if (symbol.input == "^" && underover) {
                node = AMserver.createMmlNode("mover", node);
                node.appendChild(result[0]);
            }
            else {
                node = AMserver.createMmlNode(symbol.tag, node);
                node.appendChild(result[0]);
            }
            if (typeof sym1.func != 'undefined' && sym1.func) {
                ...
                if (sym2.ttype != INFIX && sym2.ttype != RIGHTBRACKET &&
                    ...
                    result = AMserver.AMparseIexpr(str);
                    node = AMserver.createMmlNode("mrow", node);
                    node.appendChild(result[0]);
                    str = result[1];
                }
    ...
    AMparseExpr: (str, rightbracket = false) => {
        var symbol, node, result, i, newFrag = AMserver.createDocumentFragment();
        let safety = 0;
        do {
            if (safety++ > 100)
            ...
            if (symbol.ttype == INFIX && symbol.input == "/") {
                ...
                if (result[0] == null) // show box in place of missing argument
                    ...
                    AMserver.AMremoveBrackets(result[0]);
                str = result[1];
                AMserver.AMremoveBrackets(node);
                node = AMserver.createMmlNode(symbol.tag, node);
                node.appendChild(result[0]);
                newFrag.appendChild(node);
                symbol = AMserver.AMgetSymbol(str);
            ...
            || AMserver.AMnestingDepth == 0)
        && symbol != null && symbol.output != "");
        if (symbol.ttype == RIGHTBRACKET || symbol.ttype == LEFTRIGHT) {
            //    if (AMnestingDepth > 0) AMnestingDepth--;
            let res = AMserver.detectMatrix(newFrag, symbol.output);
            if (res.isMatrix) {
                let r, c, row;
                let columnlines = [];
                let table = AMserver.createMmlNode('mtable');
                for (r = 0; r < res.rows.length; r++) {
                    row = AMserver.createMmlNode('mtr');
                    for (c = 0; c < res.rows[r].length; c++) {
                        if (res.rows[r][c].length == 1 &&
                            ...
                            const cell = AMserver.createMmlNode('mtd');
        ...
        for (const node of children) {
            if (expecting === 'mrow') {
                ...
                if (
                    ...
                    node.firstChild.nodeValue.trim() !== AMserver.listseparator) {
                    return { isMatrix: false, rows: [] };
                }
                expecting = 'mrow';
            }
        ...
        for (const row of rows) {
            ...
            for (const cell of inner) {
                if (
                    ...
                    cell.firstChild.nodeValue === AMserver.listseparator) {
                    elementCount++;
    ...
    parseMath: (str) => {
        ...
        // str = str.replace(/&gt;/g, ">");
        // str = str.replace(/&lt;/g, "<");
        let frag = AMserver.AMparseExpr(str.replace(/^\s+/g, ""), false)[0];
        let node = AMserver.createMmlNode("mstyle", frag);
        node.style += "font-family:STIX Two Math;";
        if (AMserver.mathcolor != "")
            node.setAttribute("mathcolor", AMserver.mathcolor);
        if (AMserver.mathfontsize != "") {
            node.setAttribute("fontsize", AMserver.mathfontsize);
            node.setAttribute("mathsize", AMserver.mathfontsize);
        }
        // if (AMserver.mathfontfamily != "") {
        //     node.setAttribute("fontfamily", AMserver.mathfontfamily);
        //     node.setAttribute("mathvariant", AMserver.mathfontfamily);
        // }
        if (AMserver.displaystyle)
            node.setAttribute("displaystyle", "true");
        node = AMserver.createMmlNode("math", node);
        node = AMserver.createMmlNode('div', node);
        // node.style = AMserver.cancelStyle;
        if (AMserver.showasciiformulaonhover) { //fixed by djhsu so newline
